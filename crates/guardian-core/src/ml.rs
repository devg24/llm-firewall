...
impl SharedModel {
    pub fn load_from_dir(model_dir: &Path) -> Result<Self, CoreError> {
        ...
        // 1. Load Config
        let config_file =
            std::fs::File::open(&config_path).map_err(|e| CoreError::ModelLoad(e.to_string()))?;
        ...
        if id2label.is_empty() {
            ...
            let labels = [
                "O", "B-MISC", "I-MISC", "B-PERSON", "I-PERSON", "B-ORG", "I-ORG", "B-GPE", "I-GPE",
            ];
        ...
        let classifier_weight = var_builder
            .get((num_labels, config.hidden_size), "classifier.weight")
            .map_err(|e| CoreError::ModelLoad(e.to_string()))?;
        let classifier_bias = var_builder
...
pub async fn run_inference(
    ...
    text: String,
) -> Result<Vec<TokenClassification>, CoreError> {
    if text.len() > 100_000 {
    ...
    let result = tokio::time::timeout(
        ...
        tokio::task::spawn_blocking(move || -> Result<Vec<TokenClassification>, CoreError> {
            let _permit = permit;
                    ...
                    if label != "O" {
                        ...
                        if offset.0 < offset.1 && offset != (0, 0) {
                            ...
                            if let Some(word_slice) = text.get(s..e) {
                                let score_val = prob_vec
                                    .get(i + 1)
                                    .and_then(|row| row.get(pred as usize))
                                    .copied()
                                    .unwrap_or(1.0);
            ...
            results.dedup_by(|a, b| {
                a.start == b.start && a.end == b.end && a.entity_group == b.entity_group
            });
